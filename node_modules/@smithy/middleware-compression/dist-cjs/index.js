...
var compressStream = require('./compressStream');
var compressString = require('./compressString');
...
const NODE_DISABLE_REQUEST_COMPRESSION_CONFIG_OPTIONS = {
    environmentVariableSelector: (env) => config.booleanSelector(env, NODE_DISABLE_REQUEST_COMPRESSION_ENV_NAME, config.SelectorType.ENV),
    configFileSelector: (profile) => config.booleanSelector(profile, NODE_DISABLE_REQUEST_COMPRESSION_INI_NAME, config.SelectorType.CONFIG),
...
const NODE_REQUEST_MIN_COMPRESSION_SIZE_BYTES_CONFIG_OPTIONS = {
    environmentVariableSelector: (env) => config.numberSelector(env, NODE_REQUEST_MIN_COMPRESSION_SIZE_BYTES_ENV_NAME, config.SelectorType.ENV),
    configFileSelector: (profile) => config.numberSelector(profile, NODE_REQUEST_MIN_COMPRESSION_SIZE_BYTES_INI_NAME, config.SelectorType.CONFIG),
        ...
        if (CLIENT_SUPPORTED_ALGORITHMS.includes(algorithm)) {
            ...
            if (isStreaming(body)) {
                if (!streamRequiresLength) {
                    updatedBody = await compressStream.compressStream(body);
                ...
                if (bodyLength && bodyLength >= requestMinCompressionSizeBytes) {
                    updatedBody = await compressString.compressString(body);
                    isRequestCompressed = true;
                }
            }
...
const resolveCompressionConfig = (input) => {
    ...
    return Object.assign(input, {
        disableRequestCompression: client.normalizeProvider(disableRequestCompression),
        requestMinCompressionSizeBytes: async () => {
            const requestMinCompressionSizeBytes = await client.normalizeProvider(_requestMinCompressionSizeBytes)();
